...
export interface GitGraphBackend {
  ...
  getHistory(repositoryId: string, request?: GitGraphHistoryRequest): Promise<GitGraphPage>;
  getCommitDetails(
    repositoryId: string,
...
export class LocalGitBackend implements GitGraphBackend {
  ...
  async getHistory(
    repositoryId: string,
    request: GitGraphHistoryRequest = {}
  ): Promise<GitGraphPage> {
    const repository = await this.#resolveRepository(repositoryId, request.signal);
    const limit = Math.min(this.#maxPageSize, Math.max(1, request.limit ?? this.#maxPageSize));
    const refs = await this.#getRefs(repository, request.signal);
    const stashes = repository.bare ? [] : await this.#getStashes(repository, request.signal);
    ...
    const rawCommits = await this.#readCommits(repository.path, oids, request.signal);
    ...
    if (!request.cursor && request.includeWorkingTree !== false && !repository.bare && repository.head) {
      const status = await this.#getWorkingTreeChanges(repository, request.signal);
  ...
  async #resolveRepository(id: string, signal?: AbortSignal): Promise<ResolvedRepository> {
    ...
    if (!configured) {
      throw new GitGraphProtocolError("repository_not_found", `Repository ${id} is not registered.`, {
        status: 404
      });
    }
    let path: string;
    try {
      ...
      throw new GitGraphProtocolError("repository_not_found", `Repository ${id} is unavailable.`, {
        status: 404
      });
    }
    ...
    try {
      ...
      throw new GitGraphProtocolError("repository_not_found", `${path} is not a Git repository.`, {
        status: 404
      });
    }
  }
  ...
  async #readCommits(path: string, oids: readonly string[], signal?: AbortSignal): Promise<RawCommit[]> {
    ...
    while (offset < output.length) {
      ...
      if (!oid || type !== "commit" || !Number.isFinite(size)) {
        throw new GitGraphProtocolError("internal_error", `Unexpected git cat-file response: ${header}`, {
          status: 500
        });
      }
    ...
    try {
      return await new Promise<Buffer>((resolveOutput, reject) => {
        ...
        const timer = setTimeout(() => {
          child.kill("SIGKILL");
          reject(new GitGraphProtocolError("git_unavailable", "Git command timed out.", { status: 504, retryable: true }));
        }, this.#timeoutMs);
        ...
        child.on("close", (code) => {
          ...
          if (signal?.aborted) {
            ...
            reject(new GitGraphProtocolError("output_limit", "Git output exceeded the configured limit.", { status: 413 }));
          } else if (code !== 0) {
            reject(
              new GitGraphProtocolError("git_unavailable", Buffer.concat(errors).toString("utf8").trim() || "Git command failed.", {
                status: 500
              })
            );
          } else {
    ...
    try {
      ...
      try {
        ...
        if (value.code === "ENOENT") {
          throw new GitGraphProtocolError("git_unavailable", `Git executable not found: ${this.#git}`, {
            status: 503
          });
        }
        if (value.killed) {
        ...
        if (value.code === "ERR_CHILD_PROCESS_STDIO_MAXBUFFER") {
          throw new GitGraphProtocolError("output_limit", "Git output exceeded the configured limit.", {
            status: 413
          });
        }
        throw new GitGraphProtocolError(
