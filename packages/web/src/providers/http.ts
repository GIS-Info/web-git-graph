...
import { GIT_GRAPH_CONTENT_TYPE } from "@web-git-graph/protocol/http";
import type {
...
import type { GitGraphHistoryRequest, GitGraphProvider } from "../provider";
  ...
  async #request<T>(path: string, init: RequestInit): Promise<T> {
    ...
    if (!response.ok) {
      ...
      throw new GitGraphProtocolError(error?.code ?? "internal_error", error?.message ?? response.statusText, {
        ...
        details: { httpStatus: response.status, ...error?.details }
      });
    }
    return body as T;
  }
